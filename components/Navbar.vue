<template>
  <header class="flex items-center">
    ...
    <ul class="flex ml-auto items-center mt-2" v-if="!user">
      <li>
        <NuxtLink
          to="/login"
          class="inline-block bg-white border-white border hover:bg-transparent hover:text-white text-gray-700 font-light w-40 text-center px-6 py-1 text-lg rounded-full mr-4"
        >
          Sign In
        </NuxtLink>
      </li>
    ...
    <div class="flex ml-auto items-center mt-2" v-else>
...
const { user, checkUser, clean } = useAuthUser();
checkUser();
